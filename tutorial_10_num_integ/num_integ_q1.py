...
def do_analysis(f,integ_f,a,b,acc = 0.01):
    n_max = 1000
    t = integ_f(a,b)
    f_approx_simp = [integral(f,a,b,i,kind='simp') for i in range(2,n_max,20)]
    f_approx_tpz = [integral(f,a,b,i,kind='tpz') for i in range(1,n_max,10)]
    ...
    index_simp =[i for i in range(2,n_max,20)] 
    index_tpz =[i for i in range(1,n_max,10)] 
    plt.plot(index_simp,f_error_simp)
    plt.plot(index_tpz,f_error_tpz)
    plt.legend(['simpson','trapazoid'])
    plt.show()
...
def analysis_simp(f,integ_f,a,b,acc=0.01):
    ...
    f_err = []
    t = integ_f(a,b)
    while((err-acc)>1e-8 and n<n_max):
        approx = integral(f,a,b,n,kind='tpz')
        index.append(n)
        err = abs((approx-t)/t)*100
        f_err.append(err)
        n+=2 # keep this even number 
    print((index[-1]))
    print(f_err[-1])
...
print(integral(f1 , 0,1 ,64 , kind='simp'))